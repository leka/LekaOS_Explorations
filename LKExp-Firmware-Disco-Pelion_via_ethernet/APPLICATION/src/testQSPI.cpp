...
QSPI qspi_device(QSPI_FLASH1_IO0, QSPI_FLASH1_IO1, QSPI_FLASH1_IO2,
                 QSPI_FLASH1_IO3, QSPI_FLASH1_SCK,
                 QSPI_FLASH1_CSN); // io0, io1, io2, io3, sclk, ssel
...
static bool mem_ready() {
  ...
  do {
    retries--;
    if (QSPI_STATUS_OK != qspi_device.command_transfer(CMD_RDSR, -1, NULL, 0,
                                                       status_value,
                                                       STATUS_REG_SIZE)) {
...
static int write_enable() {
  ...
  if (QSPI_STATUS_OK !=
      qspi_device.command_transfer(CMD_WREN, -1, NULL, 0, NULL, 0)) {
...
static int flash_init() {
  ...
  status = qspi_device.command_transfer(CMD_RDSR, -1, NULL, 0, status_value,
                                        STATUS_REG_SIZE);
...
static int sector_erase(unsigned int flash_addr) {
  ...
  if (QSPI_STATUS_OK !=
      qspi_device.command_transfer(CMD_ERASE, (((int)flash_addr) & 0x00FFF000),
                                   NULL, 0, NULL, 0)) {
    printf("Erase failed\n");
    return -1;
  }
...
int qspiExample() {
  ...
  result = qspi_device.configure_format(
      QSPI_CFG_BUS_SINGLE, QSPI_CFG_BUS_SINGLE, QSPI_CFG_ADDR_SIZE_24,
